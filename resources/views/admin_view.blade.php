...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark menu">
  <div class="container">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link" href="{{url('area_admin/cadastrar_financa')}}">Nova finança</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="{{url('area_admin/listar_financas')}}">Gerenciar finanças</a>
        </li>
        <li class="nav-item">
...
@if($funcao == "cadastrar_financa")
  <div class="dashboard-content">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-6">
              @if(isset($msg) && $msg!=false)
                  <div class="card {{$classeAlert}}">
                    <div class="card-body">
                        <div class="text text-center">{{$msg}}</div>
                        <script>
                            setTimeout(function() {
                                window.location.href = 'http://localhost/financa-app/public/area_admin/cadastrar_financa';
                ...
                <br>  
                <div class="card">
                    <div class="card-header text-center">CADASTRAR FINANÇA</div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('cadastrarFinanca') }}" data-parsley-validate="">
                            @csrf
                            <div class="form-group">
                                <label for="email">Finança:</label>
                                <input type="text" placeholder="Descrição da finança(Ex:icms...)" name="descricao" class="form-control"  required="">
                            </div>
                            <div class="form-group">
                                <label for="password">Valor:</label>
                                <input type="text" placeholder="Valor da finança(Ex:150.55)" name="valor" class="form-control"  required="">
                            </div>
                            <br>
                            <div class="d-grid gap-2">
                                <button class="btn btn-primary" type="submit" id="btnLogar">Salvar</button>
                            </div>
                        </form>
                    </div>
                </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="POST" action="{{ route('editarFinanca') }}" data-parsley-validate="">
              @csrf
              <div class="form-group">
                  <input type="hidden" name="area" value="area_admin" class="form-control" required="">
              </div>
              <div class="form-group">
  ...
  <script>
    function excluirFinanca(event, id_financeiro) {
      ...
      if (confirm('Realmente deseja excluir esse registro?')) {
        axios.delete('{{ route('excluirFinanca', '') }}/' + id_financeiro)
        .then(response => {
            // Handle success response
            if(response.data == 1){
              alert("Finança excluida com sucesso!");
              window.location.href="../../public/area_admin/listar_financas";
            }else{
